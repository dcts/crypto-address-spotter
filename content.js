(function () {
  const cryptoAddresses = {
    ...
    "account_rdx12ywe6kvell7flpxhmdv6f55t2a3759a82kg0n0cugu0fv5p6wl3ry5": "Balrog1979",
    ...
    "account_rdx169k7mvw4069xuwalm256wkrtq73tjdmc7txvh2uty3qdn503s2f07s": "Jay XRD",
    ...
    "account_rdx169k7mvw4069xuwalm256wkrtq73tjdmc7txvh2uty3qdn503s2f07s": "@voe9e",
  ...
  function scanAndAnnotate() {
    ...
    const addressSuffixes = Object.keys(cryptoAddresses).map(addr => addr.slice(-5));
    ...
    while (node = treeWalker.nextNode()) {
      addressSuffixes.forEach(suffix => {
        if (node.nodeValue.includes(suffix)) {
          ...
          const address = Object.keys(cryptoAddresses).find(addr => addr.endsWith(suffix));
          const name = cryptoAddresses[address];

          // Check if an annotation has already been added for this suffix in this node
          if (!annotations.has(node) || !annotations.get(node).includes(suffix)) {
            ...
            const textPosition = node.nodeValue.indexOf(suffix) + suffix.length;
            ...
            annotation.style.left = `${rect.left - parentRect.left + window.scrollX}px`;
            annotation.style.top = `${rect.top - parentRect.top + window.scrollY}px`;
    ...
    // Check if annotations are still in view, remove if they are not
    annotations.forEach((suffixes, node) => {
      suffixes.forEach(suffix => {
        const annotation = node.parentNode.querySelector(`span:contains('${cryptoAddresses[suffix]}')`);
        if (annotation && !isInViewport(annotation)) {
          annotation.remove();
          annotations.get(node).splice(annotations.get(node).indexOf(suffix), 1);
